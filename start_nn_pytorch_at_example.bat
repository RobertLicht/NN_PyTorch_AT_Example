...
REM ... Ask if sync from git should be executed ....
ECHO [i] Do you want sync your files using git? (y/n)
SET /P INPUT_SYNC_GIT=Enter y or n: 
IF /I "%INPUT_SYNC_GIT%" EQU "y" (
    GOTO DOSYNCGIT
) ELSE (
    ECHO [i] Going to DONOTSYNCGIT
    GOTO DONOTSYNCGIT
)
:DOSYNCGIT
...
ECHO -----------------------------------------------------------------------------------------------------
ECHO [i] Please enter infromation about the existing virtual environment, which will be used by python
ECHO [i]    Default name of the virtual environment: %DEFAULT_VENV_NAME%
SET /P VENV_NAME=Please enter name of virtual environment (Hit RETURN to use the default): 
IF "%VENV_NAME%" EQU "" (
    GOTO DEVAULTVENVNAMENOSYNC
) ELSE (
    GOTO INPUTVENVNAMENOSYNC
)
:DEVAULTVENVNAMENOSYNC
REM Reset variable
SET VENV_NAME = ""
SET VENV_NAME=%DEFAULT_VENV_NAME%
ECHO [i] Using default virtual environment: %VENV_NAME%
...
START POWERSHELL -executionpolicy bypass -noexit -command "[console]::Title='NeuronalNetwork_PyTorch_AT_Example'; [console]::WindowWidth=140; [console]::WindowHeight=40; [console]::BufferWidth=[console]::WindowWidth"; CD %PUBLIC%/NN_PyTorch_AT_Example; ./venv_nn_pytorch_jb/Scripts/activate.ps1; Write-Host "`r`n[i] INITIALIZING - This may take some time..."; python nn_pytorch_at_example.py
GOTO SETUPFINISHED
:ABORTSETUP
COLOR 0C
ECHO Aborting launch of workflow EEVIM Tool SBT Anonymizer ... && TIMEOUT /T 60
:SETUPFINISHED
COLOR 0A
ECHO Configuration and launch of workflow EEVIM Tool SBT Anonymizer finished... && TIMEOUT /T 7
COLOR 07